<@ms.html5>
	...
	<@ms.panel>
			...
			<@ms.form  name="contentModelForm" isvalidation=true action="${base}/manager/cms/contentModel/${autoCURD}.do">
			...
			$(this).request({url:URL,method:"post",type:"json",func:function(msg) {
				if(msg != 0) {
		    		...
		    		if($("tbody tr").length==0 && msg != 1){
						...
						location.href = base+"/manager/cms/contentModel/${contentModel.cmId?default(0)}/edit.do";
					}
		    	} else {
					alert("删除字段失败");
			    	$(".closeModal").click();
